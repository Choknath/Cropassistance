 ...
 * File: app/views/farmlog/plots_create.php
 *
 * GET  → shows the "Add New Plot" form
 * POST → validates input and saves to field_plots table
 *
 * Route: GET  plots/create
 *        POST plots/store
 */
...
// ── Variables reset ────────────────────────────────────────
$errors = [];
$old    = []; // keeps form values on validation error
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Keep values so the form is pre-filled on error
    $old = compact('plot_name', 'area', 'location', 'soil_type', 'notes');
    ...
    if (empty($area) || !is_numeric($area) || (float)$area <= 0) {
        $errors[] = 'Area must be a positive number (e.g. 0.5 for half a hectare).';
    } elseif ((float)$area > 9999.99) {
        $errors[] = 'Area seems too large. Please double-check.';
    }

    if (empty($location)) {
        $errors[] = 'Location / barangay is required.';
    }

    // ── Save to database ────────────────────────────────────
    if (empty($errors)) {
        try {
            db()->table('field_plots')->insert([
                'user_id'       => $user_id,
                'plot_name'     => $plot_name,
            ...
            // Flash success and redirect back to plots list
            $_SESSION['flash_success'] = "Field plot \"{$plot_name}\" was added successfully!";
            header('Location: ' . url('plots'));
            exit;

        } catch (Exception $e) {
            $errors[] = 'Could not save the plot. Please try again.';
            error_log('Plot create error: ' . $e->getMessage());
        }
    }
}
?>
...
<head>
    ...
    <title>Add Field Plot — Smart Crop Assistant</title>
    ...
    <div class="max-w-6xl mx-auto px-5 py-3 flex items-center justify-between">
        <a href="<?= url('dashboard') ?>" class="flex items-center gap-3">
            ...
            <div class="leading-tight">
                ...
                <div class="text-xs text-gray-400">FarmLog — Add Plot</div>
            </div>
        </a>
...
<div class="bg-green-900 text-white py-10 px-4">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-2 text-green-400 text-xs mb-3">
            ...
            <span>›</span>
            <span class="text-green-200">Add New Plot</span>
        </div>
        <h1 class="text-3xl font-bold mb-1">Add Field Plot</h1>
        <p class="text-green-300 text-sm">
            Register one of your rice fields. You can add multiple plots.
        </p>
    </div>
</div>
...
<main class="max-w-2xl mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden fade-up">
        ...
        <div class="bg-gradient-to-r from-green-700 to-green-600 px-6 py-4 flex items-center gap-3">
            ...
            <div>
                <h2 class="text-white font-semibold text-base">New Field Plot Details</h2>
                <p class="text-green-200 text-xs">Fill in your plot information below</p>
            </div>
        </div>

        <!-- Form body -->
        <div class="p-7">
            <form method="POST" action="<?= url('plots/store') ?>">
                <?= csrf_field() ?>
                ...
                <div class="mb-5">
                    ...
                    <input type="text"
                           name="plot_name"
                           value="<?= htmlspecialchars($old['plot_name'] ?? '') ?>"
                           placeholder="e.g. North Field, Plot A, Bukid sa Gitna..."
                           maxlength="100"
                           class="w-full border border-gray-200 rounded-xl px-4 py-3
                                  text-sm bg-gray-50 focus:bg-white transition-colors">
                    <p class="text-xs text-gray-400 mt-1">Give it a name you'll easily recognize.</p>
                </div>

                <!-- Area + Soil Type (side by side on larger screens) -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                    ...
                    <div>
                        ...
                        <input type="number"
                               ...
                               class="w-full border border-gray-200 rounded-xl px-4 py-3
                                      text-sm bg-gray-50 focus:bg-white transition-colors">
                        <p class="text-xs text-gray-400 mt-1">1 hectare = about 10,000 m²</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="soil_type"
                            ...
                            $selected_soil = $old['soil_type'] ?? 'Clay Loam';
                            foreach ($soil_options as $s):
                            ?>
                            <option value="<?= $s ?>"
                                    <?= $selected_soil === $s ? 'selected' : '' ?>>
                                <?= $s ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                        <p class="text-xs text-gray-400 mt-1">Clay Loam is most common in PH rice fields.</p>
                    </div>
                </div>
                ...
                <div class="mb-5">
                    ...
                    <input type="text"
                           name="location"
                           value="<?= htmlspecialchars($old['location'] ?? $user['location'] ?? '') ?>"
                ...
                <div class="mb-7">
                    ...
                    <textarea name="notes"
                              rows="3"
                              placeholder="e.g. Near the irrigation canal, usually floods in rainy season..."
                              class="w-full border border-gray-200 rounded-xl px-4 py-3
                                     text-sm bg-gray-50 focus:bg-white transition-colors resize-none"><?= htmlspecialchars($old['notes'] ?? '') ?></textarea>
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit"
                            class="btn-primary flex-1 text-white font-semibold
    ...
    <!-- Tip card -->
    <div class="mt-5 bg-green-50 border border-green-100 rounded-2xl p-5">
        <p class="text-xs font-semibold text-green-800 mb-2">💡 Quick Tips</p>
        <p class="text-xs text-green-700 leading-relaxed">
            You can add as many plots as you have rice fields.
            Each plot can have multiple crop cycles tracked separately.
            Accurate area (hectares) helps calculate the correct fertilizer amounts.
        </p>
    </div>
